...
def is_food(qid, retries=3, delay=2):
    """Check if QID represents food or its subclasses."""
    if qid == "NA":
        return False
    query = f"""
    ASK {{ 
        {{ wd:{qid} (wdt:P31|wdt:P279*) wd:Q2095. }}  # Food
        UNION
        {{ wd:{qid} (wdt:P31|wdt:P279*) wd:Q1778821. }}  # Cuisine
        UNION
        {{ wd:{qid} (wdt:P31|wdt:P279*) wd:Q746549. }}  # Dish
        UNION
        {{ wd:{qid} (wdt:P31|wdt:P279*) wd:Q10675206. }}  # Ingredients
    }}
    """
    for attempt in range(retries):
        try:
            ...
            print(f"SPARQL query failed for food check on '{qid}', attempt {attempt + 1}: {e}")
            time.sleep(delay + random.uniform(0, 2))
    return False

def get_best_food_qid(term, lang):
    """Find best matching food QID, trying lowercase if needed."""
    ...
    for attempt in range(retries):
        try:
            ...
            return {
                ...
                "is_category": "Yes" if is_food(qid) else "No",
                "Alternative QIDs": "NA"
            }
        except Exception as e:
...
def fetch_wikidata_food_info(food_list, query_lang):
    """Fetch Wikidata food info with category verification."""
    results = []
    
    for term in food_list:
        qid, is_food_flag, alternative_qids = get_best_food_qid(term, query_lang)
        wikidata_info = get_wikidata_info(qid, term)
        
        # Handle is_category status based on QID presence and food check
        if qid == "NA":
            wikidata_info["is_category"] = "NA"
        else:
            wikidata_info["is_category"] = is_food_flag
            
        wikidata_info["Alternative QIDs"] = ", ".join(alternative_qids) if is_food_flag == "No" else "NA"
